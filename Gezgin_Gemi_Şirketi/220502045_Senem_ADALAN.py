...
class Gemi:
    ...
    def veritabanina_ekle(self):
        conn = baglan()
        cursor = conn.cursor()   #cursor nesnesini kullanarak sorguları çalıştırılabilir, veritabanından veri çekilebilir, veritabanına veri eklenebilir, güncellenebilir veya silinebilir. Yani temelde veritabanı işlemlerini gerçekleştirmek için kullanılan bir araçtır. 
...
def gemiler_formu(tab):
    ...
    gemiler_frame = ttk.Frame(tab)  #gemiler_frame adında bir Tkinter çerçeve nesnesi oluşturulur.Bu çerçeve, belirli bir sekme içindeki gemilere ilişkin bilgileri gösterecek olan bir bölümü temsil eder.
    gemiler_frame.pack(fill="both", expand=True)  #Ardından, oluşturulan çerçeve pack yöntemi kullanılarak Tkinter penceresine eklenir. Bu yöntem, çerçevenin nasıl yerleştirileceğini belirler. fill="both" parametresi, çerçevenin hem yatay hem de dikey olarak ana pencere boyunca genişlemesini sağlar. expand=True parametresi, pencerenin boyutu değiştirildiğinde çerçevenin de boyutunun değişmesini sağlar.
    ...
    gemi_agirlik_label.grid(column=0, row=1)  #Oluşturulan etiket, grid yöntemi kullanılarak gemiler_frame çerçevesine yerleştirilir.Burada, etiketin 0. sütunda ve 1. satırda yer almasını sağlar.
    gemi_agirlik_entry = ttk.Entry(gemiler_frame)  #ttk.Entry sınıfı, kullanıcıdan metin girmesini istediğiniz bir giriş alanı oluşturur. Bu giriş alanı, gemiler_frame çerçevesine yerleştirilir.
    gemi_agirlik_entry.grid(column=1, row=1, padx=(0, 50))  #Oluşturulan giriş alanı, grid yöntemi kullanılarak gemiler_frame çerçevesine yerleştirilir.Burada, giriş alanının 1. sütunda ve 0. satırda yer almasını sağlar.
    ...
    gemi_tip_combobox = ttk.Combobox(gemiler_frame, values=["Yolcu Gemisi", "Petrol Tankeri", "Konteyner Gemisi"])  #Öncelikle, ttk.Combobox sınıfı kullanılarak bir kombinasyon kutusu oluşturulur. Oluşturulan kombinasyon kutusu gemiler_frame çerçevesine yerleştirilir.values parametresi, kullanıcıya sunulan seçeneklerin bir listesini belirtir.
    gemi_tip_combobox.grid(column=1, row=3, padx=(0, 50))   #Oluşturulan kombinasyon kutusu, grid yöntemi kullanılarak gemiler_frame çerçevesine yerleştirilir.Burada, kombinasyon kutusunun 1. sütunda ve 3. satırda yer almasını sağlar.
    ...
    gemi_tip_combobox.bind("<<ComboboxSelected>>", tip_secildi)   #Burada <<ComboboxSelected>> olayı, kullanıcının kombinasyon kutusundan bir öğe seçtiği zaman tetiklenir.bind() yöntemi, belirli bir olayın gerçekleştiğinde çağrılacak bir fonksiyonu belirlemek için kullanılır. Yani, kombinasyon kutusunda bir öğe seçildiğinde belirtilen tip_secildi adlı bir fonksiyon çalıştırılacaktır.
...
tab_control = ttk.Notebook(root)  #İlk olarak, ttk.Notebook sınıfı kullanılarak bir sekme denetimi oluşturulur. Bu, Tkinter penceresinde birden fazla sekme (tab) içeren bir arayüz oluşturmayı sağlar.
gemiler_tab = ttk.Frame(tab_control)  #Ardından, ttk.Frame sınıfı kullanılarak bir çerçeve (frame) oluşturulur. Bu çerçeve, ilk sekme (tab) içeriğini temsil eder. Yani, bu çerçeve, kullanıcıya gemilerle ilgili bilgileri göstermek için kullanılacak olan içeriği barındırır.
tab_control.add(gemiler_tab, text='Gemiler')  #tab_control sekme denetimine gemiler_tab çerçevesini ekler.
gemiler_formu(gemiler_tab)   #gemiler_formu adlı bir fonksiyon çağrılır ve bu fonksiyon gemiler_tab çerçevesine gemi formunu ekler. Bu, gemi formunun gemiler_tab çerçevesinde görüntülenmesini sağlar.
tab_control.pack(expand=1, fill="both")   #pack() yöntemi kullanılarak, sekme denetiminin nasıl yerleştirileceğini belirtilir. expand=1 parametresi, sekme denetiminin boş alanı doldurmasını sağlar. fill="both" parametresi, sekme denetiminin hem yatay hem de dikey olarak ana pencere boyunca genişlemesini sağlar.
